...
const AdvertisingBlock = ({ data }) => {
    ...
    const handleCheck = () => {
        setCheckLoad(true);
        handleRedactProduct({ ...data, new: false }).finally(() =>
            setCheckLoad(false),
        );
    };
    ...
    const handleRedact = (show) => {
        setRedactLoad(true);
        handleRedactProduct({ ...data, show: show }).finally(() =>
            setRedactLoad(false),
        );
    };
    ...
    const handleDelete = () => {
        setDeleteLoad(true);
        handleDeleteRealtime(`/${data.idGroup}/${data.idAdv}`).finally(() =>
            setDeleteLoad(false),
        );
    };

    if (!token && !data.view) return;
    ...
    return (
        <div className={'AdvertisingBlock'}>
            <div className={'content'}>
                ...
                <ButtonGroup hidden={!token} size={'sm'} className={'mt-2'}>
                    {data?.show ? (
                        <Button
                            disabled={redactLoad}
                            variant={'secondary'}
                            onClick={() => handleRedact(false)}
                        >
